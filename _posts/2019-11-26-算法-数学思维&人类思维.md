...
![](https://www.echoheart.cn/img/math1-1.png)
...
不包括任何除了`diedai`执行以外的操作, 比如`3 * diedaidiedai(1, n, 1)`因为如果这样的话, 就会出现`diedai`这个函数使用了3这个变量, 那么就必须将函数入栈, 来保存函数的执行环境, 所以说这两个条件必须严格符合才是, **尾递归**
