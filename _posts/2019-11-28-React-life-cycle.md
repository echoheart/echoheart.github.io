...
`props`发生变化之后或者父组件重新渲染都会触发这个生命周期函数
在这个函数中可以通过`nextProps`获取到变化之后`props`, 通过`this.props`获取到之前的`props`, 在这可以进行`setState`
...
不能在这里`setState`, 因为能走到这个钩子函数说明了`shouldComponentUpdate`返回了`true`, 这时下一个`state`状态已经确定, 马上就要执行`render`函数了, 
否则可能会导致整个证明周期混乱
